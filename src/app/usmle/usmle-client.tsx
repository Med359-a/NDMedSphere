          ...
          <div className={`grid gap-10 ${isAdmin ? "lg:grid-cols-2 lg:items-start" : ""}`}>
            ...
            {isAdmin && (
              <div className="animate-fade-up stagger-2 rounded-2xl border border-[#dbe8f5] bg-[#f4f8fd] p-7 shadow-sm dark:border-white/10 dark:bg-[#0d1b30]">
                <div className="text-sm font-semibold text-[#0c2d6b] dark:text-white">Add Resource</div>
                <form onSubmit={onCreate} className="mt-5 grid gap-4">
                  ...
                  <div className="grid gap-1.5">
                    <label className="text-xs font-semibold text-[#7a90ab] dark:text-slate-400" htmlFor="description">Description (optional)</label>
                    <textarea
                  ...
                  <div className="grid gap-1.5">
                    <label className="text-xs font-semibold text-[#7a90ab] dark:text-slate-400" htmlFor="file">File — PDF or Image (optional)</label>
                    <input
      ...
      <section className="bg-[#f4f8fd] pb-20 dark:bg-[#0d1b30]">
        <Container className="py-16">
          <div className="flex items-end justify-between gap-6 mb-8">
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-[#0c2d6b] dark:text-white">Study Materials</h2>
              <p className="mt-1 text-sm text-[#7a90ab] dark:text-slate-400">Click any card to view the full resource.</p>
            </div>
            <button
              type="button"
          ...
          {load.status === "loading" && (
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-56 animate-pulse rounded-2xl border border-[#dbe8f5] bg-white dark:border-white/10 dark:bg-[#0f2040]" />
              ))}
            </div>
          )}
          ...
          {load.status !== "loading" && items.length === 0 && (
            <div className="rounded-2xl border border-[#dbe8f5] bg-white p-16 text-center shadow-sm dark:border-white/10 dark:bg-[#0f2040]">
              ...
              <div className="text-base font-semibold text-[#0c2d6b] dark:text-white">No resources yet</div>
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {items.map((item, i) => (
                <button
                  ...
                  className="animate-fade-up group relative flex flex-col cursor-pointer overflow-hidden rounded-2xl border border-[#dbe8f5] bg-white p-0 text-left shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl dark:border-white/10 dark:bg-[#0f2040]"
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-semibold leading-snug text-[#0c2d6b] dark:text-white group-hover:text-[#1666d1] dark:group-hover:text-blue-400 transition-colors">
                        {item.title}
                      </h3>
                      {isAdmin && (
                        <button
