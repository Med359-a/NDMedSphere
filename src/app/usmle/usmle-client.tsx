...
export function UsmleClient() {
  ...
  const refresh = React.useCallback(async () => {
    try {
      ...
      setLoad({
        status: "error",
        message: e instanceof Error ? e.message : "Failed to load resources.",
      });
    }
  }, []);
  ...
  async function onDelete(id: string) {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/usmle?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const payload = (await res.json().catch(() => null)) as { error?: string } | null;
          ...
          <div className={`grid gap-8 ${isAdmin ? "lg:grid-cols-2 lg:items-end" : ""}`}>
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-pretty text-lg leading-8 text-zinc-700 dark:text-zinc-200">
                Study resources, notes, and exam prep materials.
              </p>
            </div>
            ...
            {isAdmin ? (
              <div className="rounded-[2rem] border border-black/10 bg-white/60 p-8 shadow-sm backdrop-blur dark:border-white/15 dark:bg-zinc-950/50">
                <div className="text-sm font-semibold">Add resource</div>
        ...
        <Container className="py-16">
          ...
          {load.status !== "loading" && items.length === 0 ? (
            <div className="mt-10 rounded-[2rem] border border-black/10 bg-white/60 p-10 text-center shadow-sm backdrop-blur dark:border-white/15 dark:bg-zinc-950/50">
              <div className="text-lg font-semibold">No resources yet</div>
                  ...
                  {item.fileId && item.fileType === "image" ? (
                    ...
                    <div className="mt-4 flex aspect-video w-full items-center justify-center rounded-xl bg-zinc-100/50 dark:bg-zinc-800/50">
                      <span className="text-zinc-400 dark:text-zinc-600">
                        {item.fileType === "pdf" ? "PDF Document" : "No Image"}
                      </span>
                    </div>
                  )}
